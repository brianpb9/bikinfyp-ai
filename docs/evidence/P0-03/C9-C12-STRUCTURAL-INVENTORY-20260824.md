...
| `REFERENCE_IDENTITY_CHANGED` | Proposal only | No such reason/status occurs in production or tests. Current failures use `REF_MISSING`, `REF_HASH_MISMATCH`, or `REF_MANIFEST_INVALID`; A6 returns its existing generic bad-request envelope. |
...
|---|---|---|---|---|---|---|---|
| E3 | `app/api/products/[id]/route.ts:13-80`, `PATCH` | Retail metadata edit | Job metadata remains the admission `job_product_snapshot`; product row changes only | HTTP success; no snapshot-change reason | Yes, only through later W2 | `http-photo-mutation-resume-w2.test.ts:206`; `product-truth-worker-reference.test.ts:686` | Explicit proposed reason/status and A6 route counterexample |
| E5 | `app/api/products/[id]/photos/route.ts:147-174`, `DELETE` via `lib/retail-product-images.ts` | Retail image delete | Installed manifest remains authoritative; missing job-owned bytes fail closed | `REF_MISSING` when durable bytes are absent; mutation itself succeeds | Yes, only through later W2 | `http-photo-mutation-resume-w2.test.ts:270,319,352` | Admission-to-first-install mutation case; proposed identity-change reason |
...
| Reference install/reuse | `lib/job-reference-manifest.ts:46-125`; PG writer `lib/postgres/jobs.ts:53-82`; SQLite writer `lib/worker.ts:202-230` | First install, concurrent install, reuse | After installation: immutable ordered identity and job-owned bytes. Before installation: candidate comes from current product images | `REF_MISSING`, `REF_HASH_MISMATCH`, `REF_MANIFEST_INVALID` | Yes, only after materialization succeeds | Helper concurrency/crash suite plus W1/W2 reorder/delete/add tests | Admission-bound identity policy is unresolved |
...
Product metadata writers include retail E3 plus D1 wrapper paths in
`lib/postgres/product-persona-script.ts:112-140`, and org E7. Image-list writers
are E4/E5 through `lib/retail-product-images.ts:19-63` and E8/E9 through
`lib/postgres/smoke-runtime.ts:307-397`. Script-side direct job inserts are
...
for admission → queue delay → E5/E9 mutation → first worker. It should preserve
the current reason codes unless a separate approved work order changes them.
