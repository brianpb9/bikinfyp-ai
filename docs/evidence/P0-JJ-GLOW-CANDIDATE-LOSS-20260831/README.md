...
`INCIDENT.json` separates direct observations from inference and fixes the current boundary at `FOUNDER_DECISION_REQUIRED`. `SOURCE-MANIFEST.json` pins the SHA-256 of ten safe source receipts, including the exact authority relay and original BytePlus blocker receipt, and `verify.mjs` derives the boundary by crossing those receipts instead of trusting the summary alone. No database, Redis, R2, provider, or production mutation was made while collecting this evidence.
