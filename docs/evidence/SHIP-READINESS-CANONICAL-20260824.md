...
|---|---|---|---|
| P0-B2 runtime classification web | **VERIFIED_MANAGED: capable** at exact `73280ffa342945dc08cee2fc664956975c8d5735`; Docker deploy `dep-da63g43tqb8s739gkasg` live, classifier managed smoke positive | Closed for staging runtime capability; production remains separately unauthorized | `managed-classifier-retry-20260824/`: exact SHA, 35/35 migrations, sandbox/non-live, all binaries+OCR+smoke true, sampled maintenance, zero task-window aggregate, DB allowlist restored; safe admission-only canary NOT_RUN because no safe non-paid route exists |
