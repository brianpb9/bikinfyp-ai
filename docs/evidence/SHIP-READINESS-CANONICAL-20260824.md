...
|---|---|---|---|
| P0-B2 runtime classification web | **VERIFIED_MANAGED: capable** at exact `73280ffa342945dc08cee2fc664956975c8d5735`; web deploy `dep-da63g43tqb8s739gkasg` and exact worker replay deploy `dep-da645pu1egvs73a1fljg` live, classifier managed smoke positive | Closed for staging runtime capability; production remains separately unauthorized | `managed-classifier-retry-20260824/`: retained pre-resume worker-suspended fingerprint/queue baseline, immediate+sustained exact-deploy parity, 35/35 migrations, sandbox/non-live, all binaries+OCR+smoke true, DB allowlist restored, and post-replay production service objects equivalent to pre-task. The old narrow zero-money aggregate is explicitly unproven and not relied upon; safe admission-only canary NOT_RUN because no safe non-paid route exists |
