...
|---|---|---|---|
| P0-B2 runtime classification web | **VERIFIED_MANAGED: incapable**; exact-SHA Docker canary attempted and safely rolled back after build-time `AUTH_SECRET` boundary failure | Secret-safe Docker build contract, then repeat managed staging canary | `managed-web-classifier-canary-20260824/`: failed deploy `dep-da62h4ajobas738bkbug`, live rollback `dep-da62i9ek1f9s738r52r0`; still requires Render image build, sustained health `mampu=true`, and zero-money positive canary |
