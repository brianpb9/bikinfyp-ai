...
|---|---|---|---|
| P0-B2 runtime classification web | **VERIFIED_MANAGED: capable** at exact `73280ffa342945dc08cee2fc664956975c8d5735`; web deploy `dep-da63g43tqb8s739gkasg` and explicit worker deploy `dep-da63u33bc2fs73as12j0` live, classifier managed smoke positive | Closed for staging runtime capability; production remains separately unauthorized | `managed-classifier-retry-20260824/`: web+worker exact parity, 35/35 migrations, sandbox/non-live, all binaries+OCR+smoke true, controlled full-table parity fingerprints and empty queues, DB allowlist restored. The old narrow zero-money aggregate is explicitly unproven and not relied upon; safe admission-only canary NOT_RUN because no safe non-paid route exists |
