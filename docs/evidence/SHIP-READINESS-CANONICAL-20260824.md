...
|---|---|---|---|
| P0-B2 runtime classification web | **VERIFIED_MANAGED: incapable**; exact-SHA Docker build failed at build-time `AUTH_SECRET`; final staging state is restored/live/healthy, while interval-level fail-safe evidence is incomplete | Secret-safe Docker build contract, then repeat managed staging canary with contemporaneous raw captures | `managed-web-classifier-canary-20260824/`: failed deploy `dep-da62h4ajobas738bkbug`, final live state `dep-da62i9ek1f9s738r52r0`; no canary result exists and maintenance continuity/full non-mutation remain not independently proven |
