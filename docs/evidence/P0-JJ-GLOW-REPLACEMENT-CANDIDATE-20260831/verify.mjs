...
const directPathReceipt = readJson("RAW-DIRECT-PATH-JOB-LOG.json");
...
const directPathCommittedBytes = fs.readFileSync(path.join(dir, directPathReceipt.source_file));
assert.equal(directPathCommittedBytes.at(-1), 0x0a);
const directPathRawBytes = directPathCommittedBytes.subarray(0, directPathCommittedBytes.length - 1);
const directPathLogs = JSON.parse(`[${directPathRawBytes.toString("utf8").replaceAll("}{", "},{")}]`);
...
const directPathLaunches = rawWindow.filter((job) => job.startCommand === jobs.canonical_admission_signature.direct_path_start_command);
assert.equal(directPathLaunches.length, 1);
const [directPathLaunch] = directPathLaunches;
assert.equal(sha(Buffer.from(directPathLaunch.startCommand)), jobs.canonical_admission_signature.direct_path_start_command_sha256);
assert.equal(directPathLaunch.id, directPathReceipt.job_id);
assert.equal(directPathLaunch.serviceId, directPathReceipt.service_id);
assert.equal(directPathLaunch.startCommand, directPathReceipt.exact_start_command);
assert.equal(directPathLaunch.status, "failed");
assert.equal(rawMatchingAdmissions.length + directPathLaunches.length, 2);
...
assert.equal(sha(Buffer.from(directPathReceipt.query)), directPathReceipt.query_sha256);
assert.match(directPathReceipt.query, new RegExp(directPathLaunch.id));
assert.equal(sha(directPathRawBytes), directPathReceipt.raw_provider_response_sha256);
assert.ok(directPathLogs.length > 0);
assert.ok(directPathLogs.every((entry) => entry.labels.some((label) => label.name === "resource" && label.value === directPathLaunch.id)));
assert.ok(directPathLogs.every((entry) => Date.parse(entry.timestamp) >= Date.parse(directPathLaunch.startedAt)
  && Date.parse(entry.timestamp) <= Date.parse(directPathLaunch.finishedAt)));
const directMessages = directPathLogs.map((entry) => entry.message);
assert.ok(directMessages.includes("Error: Cannot find module '/srv/app/scripts/staging-jj-glow-candidate.cjs'"));
assert.ok(directMessages.includes("  code: 'MODULE_NOT_FOUND',"));
assert.ok(directMessages.includes("  requireStack: []"));
assert.equal(directMessages.some((message) => message.includes("JJ_GLOW_CANDIDATE_PASS")), false);
assert.equal(directMessages.some((message) => message.includes("JJ_GLOW_CANDIDATE_FAIL")), false);
assert.equal(directPathReceipt.result, "PRE_EXECUTION_BOOTSTRAP_FAILURE");
assert.equal(incident.replacement_attempt.control_plane_runner_launch_count, 2);
assert.equal(incident.replacement_attempt.direct_path_launch_count, directPathLaunches.length);
assert.equal(incident.replacement_attempt.direct_path_pre_execution_bootstrap_failure_count, 1);
assert.equal(incident.replacement_attempt.embedded_runner_launch_count, rawMatchingAdmissions.length);
assert.equal(incident.replacement_attempt.candidate_code_execution_count, "AT_MOST_1_UNPROVEN");
assert.equal(incident.replacement_attempt.candidate_creation_attempt_count, 1);
assert.equal(incident.replacement_attempt.candidate_code_replay_performed, false);
...
console.log("REPLACEMENT_RUNNER_LAUNCH_COUNT=2");
console.log("DIRECT_PATH_RUNNER_INVOCATION_COUNT=1");
console.log("EMBEDDED_RUNNER_INVOCATION_COUNT=1");
console.log("DIRECT_PATH_PRE_EXECUTION_BOOTSTRAP_FAILURE_COUNT=1");
console.log("CANDIDATE_CREATION_ATTEMPT_COUNT=1");
