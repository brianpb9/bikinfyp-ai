...
2. Copy the ten exact, secret-safe PASS/DONE source messages from runtime
   `.agent-bus/archive` into immutable `BUS-SOURCE-MESSAGES.json`; compare every
   field byte-for-byte, then require one Reviewer PASS and one Builder DONE per
   accepted task at its exact SHA.
...
6. Validate every scoped JSON with `jq empty`, every Markdown relative link
   ...
   phrases across both canonical documents, SHA ancestry, exact secret-value
   absence, manifest checksums, and `git diff --check`.
