...
const expectedPriceFields = ["currency","canary_package_id","canary_package_description","credited_entitlement","customer_price_idr","maximum_authorized_spend_idr","selected_payment_channel","total_variable_cogs_idr","total_variable_cogs_source","total_variable_cogs_effective_at","provider_fee_basis","tax_basis","expected_net_settlement_idr","derived_gross_margin_idr","approved_test_loss_idr_if_negative","approved_by","approval_reference"];
...
if (contract.slots.P.state !== "BLOCKED_PRE_CANARY_ECONOMICS_AND_MERCHANT_CHANNEL_READINESS" || JSON.stringify(contract.slots.P.phase_dependencies) !== JSON.stringify(["G.PRE_CANARY_ECONOMICS","G.MERCHANT_CHANNEL_READINESS"])) fail("payment canary readiness");
if (contract.slots.G.state !== "PRE_CANARY_ECONOMICS_MISSING_CANARY_NOT_READY" || contract.slots.G.phases?.PRE_CANARY_ECONOMICS?.state !== "MISSING_CHEAPEST_SKU_60000_EXCEEDS_50000_CAP" || contract.slots.G.phases?.MERCHANT_CHANNEL_READINESS?.state !== "MISSING" || contract.slots.G.phases?.PAYMENTS_GO_LIVE?.state !== "NOT_DECIDED_CANARY_REQUIRED_FIRST" || JSON.stringify(contract.slots.G.phases.PAYMENTS_GO_LIVE.depends_on) !== JSON.stringify(["P","R"])) fail("payment phase model");
if (JSON.stringify(contract.payment_sequence?.order) !== JSON.stringify(["PRE_CANARY_ECONOMICS_AND_MERCHANT_CHANNEL_READINESS","ONE_CLOSED_CANARY_SETTLEMENT_RECONCILIATION","SEPARATE_PAYMENTS_GO_LIVE_DECISION"]) || contract.payment_sequence?.canary_may_execute_before_phase_1 !== false || contract.payment_sequence?.go_live_may_be_decided_before_phase_2_pass !== false || contract.payment_sequence?.pre_canary_required_fields_source !== "FOUNDER-80-DIRECTION.json#price_cogs_required_fields") fail("payment sequence ordering");
