...
expected_runner_sha="${EXPECTED_EVIDENCE_RUNNER_SHA:?EXPECTED_EVIDENCE_RUNNER_SHA is required}"
...
test "$source_sha" = "$expected_runner_sha"
...
cleanup(){
  ...
  if test -n "$container_id"; then
    ...
    if test "$export_ok" = true && ! node -e 'const fs=require("fs");const [target,appSha,runnerSha,containerId,imageId]=process.argv.slice(1);const receipt=JSON.parse(fs.readFileSync(target,"utf8"));const evidence=receipt?.evidence_runner;const launch=evidence?.launch;if(receipt.exact_sha!==appSha||evidence?.source?.commit!==runnerSha||launch?.source_sha!==runnerSha||launch?.container_id!==containerId||launch?.image_id!==imageId||launch?.config_image!==imageId)process.exit(1)' \
      "$export_stage/receipt.json" "$expected_sha" "$expected_runner_sha" "$container_id" "$image_id"; then
      export_ok=false
    fi
...
container_id="$(docker create --name "$container_name" \
  ...
  --env "EXPECTED_EVIDENCE_RUNNER_SHA=$expected_runner_sha" \
