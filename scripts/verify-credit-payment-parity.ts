...
if (mode === "sqlite") {
  ...
  const first = creditTopup({ userId: user.id, packageId: "senyap5", gateway: "stub", gatewayRef: "webhook-1", rawPayload: { a: 1 } }); const second = creditTopup({ userId: user.id, packageId: "senyap5", gateway: "stub", gatewayRef: "webhook-1", rawPayload: { a: 2 } });
  db.prepare("INSERT INTO payments (id,user_id,gateway,gateway_ref,amount_idr,credits,status,raw_payload,created_at) VALUES (?,?,?,?,?,?,?,?,?)").run("pending", user.id, "midtrans", "checkout-1", 25000, 25000, "pending", JSON.stringify({ package_id: "senyap5" }), "2026-08-01T00:00:02.000Z");
  const pending = creditTopup({ userId: user.id, packageId: "senyap5", gateway: "midtrans", gatewayRef: "checkout-1", rawPayload: { transaction_status: "settlement" } });
  ...
  try {
    ...
    await repo.creditTopup({ userId: user.id, packageId: "senyap5", gateway: "stub", gatewayRef: "webhook-1", rawPayload: { a: 1 } }); const second = await repo.creditTopup({ userId: user.id, packageId: "senyap5", gateway: "stub", gatewayRef: "webhook-1", rawPayload: { a: 2 } });
    await repo.createCheckout({ userId: user.id, gateway: "midtrans", gatewayRef: "checkout-1", packageId: "senyap5" }); const pending = await repo.creditTopup({ userId: user.id, packageId: "senyap5", gateway: "midtrans", gatewayRef: "checkout-1", rawPayload: { transaction_status: "settlement" } });
    await repo.createCheckout({ userId: user.id, gateway: "midtrans", gatewayRef: "failed-1", packageId: "senyap5" }); const failed = await repo.markPaymentFailed("midtrans", "failed-1", { transaction_status: "expire" });
    ...
    const webhookRace = await Promise.all(Array.from({ length: 6 }, () => repo.creditTopup({ userId: user.id, packageId: "senyap5", gateway: "stub", gatewayRef: "race-webhook" }))); assert.equal(webhookRace.filter((result) => !result.duplicated).length, 1, "webhook paralel harus mengkredit sekali");
    await assert.rejects(() => repo.creditTopup({ userId: "missing-user", packageId: "senyap5", gateway: "stub", gatewayRef: "rollback-ref" })); const rolledBack = await pool.query("SELECT count(*)::int AS n FROM payments WHERE gateway_ref = 'rollback-ref'"); assert.equal(rolledBack.rows[0].n, 0, "FK gagal harus rollback payment");
