...
export function completeRequiredNsfwFormats(rows) {
  const present=new Set(rows.map((row)=>row.format));
  return [...rows,...Object.keys(NSFW_TARGET_BY_FORMAT).filter((format)=>!present.has(format)).map((format)=>({
    format,sukses:0,ditolak_konten:0,gagal_semua:0,
  }))];
}
...
export function summarizeNsfwAggregates(rows) {
  return rows.map((row)=>{
    ...
    return {format:row.format,success,rejected,otherFailures:allFailed-rejected,launched,rate,target,
      thresholdStatus:target===undefined?"UNSCOPED":launched===0?"NO_DATA":rate<=target?"PASS":"FAIL"};
  });
}

export function nsfwReportExitCode(summaries) {
  return summaries.some((row)=>row.thresholdStatus==="FAIL"||row.thresholdStatus==="NO_DATA")?1:0;
}
