...
function snapshotMatchesCurrentC5(row:unknown):boolean {
  if (!row || typeof row !== "object") return false;
  const value=row as {job_product_snapshot?:unknown;category?:unknown;category_review_version?:unknown};
  try {
    const snapshot=JSON.parse(String(value.job_product_snapshot ?? "")) as Record<string,unknown>;
    return snapshot.version===4 && snapshot.category===value.category
      && snapshot.categoryReviewVersion===value.category_review_version;
  } catch { return false; }
}
...
export async function fileBelongsToUser(relPath:string,userId:string):Promise<boolean>{
  ...
  if(deps.postgresRuntimeEnabled()){
    ...
    const generated=await pool.query(`
      SELECT j.job_product_snapshot,p.category,p.category_review_version
        FROM outputs o JOIN jobs j ON j.id=o.job_id JOIN products p ON p.id=j.product_id
        WHERE o.video_url=$1
      ...
      UNION ALL
      SELECT j.job_product_snapshot,p.category,p.category_review_version
        FROM job_shots js JOIN jobs j ON j.id=js.job_id JOIN products p ON p.id=j.product_id
    ...
    if (generated.rows?.some(snapshotMatchesCurrentC5)) return true;
    const unbound=await pool.query(`
      ...
      LIMIT 1`,[relPath,userId]);
    return Boolean(unbound.rowCount);
  }
  const db=deps.getDb();
  const c5=`p.category_review_state='CLEAR' AND p.category_review_reason IS NULL
    AND p.category_review_version>=1 AND p.category IN ${CANONICAL_CATEGORY_SQL}`;
  const output=db.prepare(`SELECT j.job_product_snapshot,p.category,p.category_review_version
    FROM outputs o JOIN jobs j ON j.id=o.job_id JOIN products p ON p.id=j.product_id
    WHERE o.video_url=? AND j.user_id=? AND ${c5} LIMIT 1`).get(relPath,userId);
  if(snapshotMatchesCurrentC5(output))return true;
  const shot=db.prepare(`SELECT j.job_product_snapshot,p.category,p.category_review_version
    FROM job_shots js JOIN jobs j ON j.id=js.job_id JOIN products p ON p.id=j.product_id
    WHERE (js.storage_key=? OR js.thumb_key=?) AND j.user_id=? AND ${c5} LIMIT 1`).get(relPath,relPath,userId);
  if(snapshotMatchesCurrentC5(shot))return true;
