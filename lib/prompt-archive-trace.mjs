...
export function verifyPromptArchiveTrace(input) {
  ...
  for (const requestRaw of requests) {
    ...
    nonEmpty(request.payload_sha256, "REQUEST_PAYLOAD_SHA256");
    if (!/^[0-9a-f]{64}$/.test(request.payload_sha256)) throw new Error("REQUEST_PAYLOAD_SHA256_INVALID");
    const archivedShot = shots.find((shot) => shot.idx === request.shot_index);
    if (!archivedShot || request.payload_sha256 !== archivedShot.providerPayloadSha256) {
      throw new Error("REQUEST_ARCHIVE_PAYLOAD_MISMATCH");
    }
