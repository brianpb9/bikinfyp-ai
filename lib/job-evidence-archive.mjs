...
export function createJobEvidenceArchiveReader(archiveRoot, jobId) {
  ...
  const lexicalJobDirectory = path.join(root, "jobs", jobId, "evidence");
  const jobDirectory = fs.realpathSync(lexicalJobDirectory);
  if (jobDirectory !== lexicalJobDirectory) throw new Error("JOB_ARCHIVE_DIRECTORY_NOT_CANONICAL");
