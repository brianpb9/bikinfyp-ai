...
export function verifyBrandAntiSlopEvidence(input, trustedRuntime) {
  ...
  if (!Number.isFinite(artifact.duration_ms) || artifact.duration_ms <= 0) throw new Error("ARTIFACT_DURATION_INVALID");
  ...
  if (extraction.artifact_duration_ms !== artifact.duration_ms) throw new Error("EXTRACTION_ARTIFACT_DURATION_MISMATCH");
  ...
  for (const raw of extraction.shot_inventory) {
    ...
    if (!Number.isFinite(shot.start_ms) || !Number.isFinite(shot.end_ms)
        || shot.start_ms < 0 || shot.end_ms <= shot.start_ms || shot.end_ms > artifact.duration_ms) {
      throw new Error("SHOT_INVENTORY_TEMPORAL_BOUNDS_INVALID");
    }
    shots.set(shot.shot_index, shot);
  }
  for (let index = 0; index < artifact.shot_count; index++) {
    if (!shots.has(index)) throw new Error("SHOT_INVENTORY_NOT_CONTIGUOUS");
    const shot = shots.get(index);
    if (index === 0 && shot.start_ms !== 0) throw new Error("SHOT_INVENTORY_TEMPORAL_GAP");
    if (index > 0 && shot.start_ms !== shots.get(index - 1).end_ms) throw new Error("SHOT_INVENTORY_TEMPORAL_GAP");
    if (index === artifact.shot_count - 1 && shot.end_ms !== artifact.duration_ms) throw new Error("SHOT_INVENTORY_TEMPORAL_GAP");
  }
  ...
  for (const raw of extraction.frames) {
    ...
    const shot = shots.get(frame.shot_index);
    if (frame.artifact_timestamp_ms < shot.start_ms || frame.artifact_timestamp_ms >= shot.end_ms) throw new Error("FRAME_OUTSIDE_SHOT_BOUNDS");
  ...
  for (const shot of shots.values()) {
    const first = frames.get(`${shot.shot_index}:first`);
    const beat = frames.get(`${shot.shot_index}:beat`);
    if (first.artifact_timestamp_ms !== shot.start_ms) throw new Error("FIRST_FRAME_NOT_AT_SHOT_START");
    if (beat.artifact_timestamp_ms <= first.artifact_timestamp_ms || beat.frame_sha256 === first.frame_sha256) {
      throw new Error("BEAT_FRAME_NOT_DISTINCT_LATER_FRAME");
    }
  }
  const extractionPayload = {
    job_id: jobId, product_id: productId, subject_id: subjectId, artifact_sha256: artifactSha,
    artifact_storage_key: artifact.storage_key, artifact_duration_ms: artifact.duration_ms,
    extractor: extraction.extractor, created_at: extraction.created_at,
  ...
  for (const code of ["QC-03", "QC-10"]) {
    const codeField = code.replace("-", "_");
    ...
    const finalReceipt = object(rows[0].receipt, `${codeField}_RECEIPT`);
    const finalReceiptKey = text(finalReceipt.storage_key, `${codeField}_RECEIPT_KEY`);
    if (!finalReceiptKey.startsWith(`jobs/${jobId}/evidence/final-qc/`)) throw new Error(`${code.replace("-", "_")}_RECEIPT_NOT_JOB_OWNED`);
    const finalReceiptSha = sha(finalReceipt.sha256, `${codeField}_RECEIPT`);
    const finalPayload = object(finalReceipt.payload, `${codeField}_RECEIPT_PAYLOAD`);
    ...
    const archivedFinal = trustedArchiveObject(runtime, finalReceiptKey, finalReceiptSha, `${codeField}_RECEIPT`);
