...
const REVIEW_JUJUR: CopyFn[] = [
  (c) => ({ hook: `[serious] Jujur ya, awalnya ${c.reg.me} kira ini bakal biasa aja`, demo: `[short pause] Nah, pada ${c.produk}, ${c.proof} bisa diperiksa; harganya ${c.harga}, kecocokan tetap personal sih`, cta: `Penilaiannya ada di keranjang kuning dulu ya` }),
...
const UNBOXING: CopyFn[] = [
  (c) => ({ hook: `[excited] Nah, paket ${c.produk} akhirnya sampai dan siap dibuka`, demo: `[short pause] Cocokkan isinya satu per satu dengan daftar; periksa ${c.proof} ya`, cta: `Isi paketnya ada di keranjang kuning ya` }),
...
const DISKON_GEDE: CopyFn[] = [
  ...
  (c) => ({ hook: `[fast] Sumpah, [giggles] angka ${c.harga} perlu dibandingkan dulu`, demo: `[short pause] Nah, periksa bagian ${c.proof} dari ${c.produk}; harganya ${c.harga}, lalu nilai sendiri sih`, cta: `Harganya ada di keranjang kuning deh` }),
];
...
const BUAT_KAMU_YANG: CopyFn[] = [
  ...
  (c) => ({ hook: `[fast] Eh, khusus yang sering ${c.aktivitas}, cek detail ini`, demo: `[long pause] ${c.produk} menampilkan ${c.proof} tanpa dilebihkan loh`, cta: `Kalau itu kamu, buka detailnya ada di keranjang kuning deh` }),
...
const SPILL_RAHASIA: CopyFn[] = [
  ...
  (c) => ({ hook: `[whispers] ${c.reg.sapaan}, rahasianya ada pada cara memeriksa lebih dekat sih`, demo: `[long pause] Nah, cocokkan atribut ${c.proof} milik ${c.produk} dengan kebutuhanmu loh`, cta: `Bocorannya ada di keranjang kuning ya` }),
...
const T04: CopyFn[] = [
  (c) => ({ hook: `[excited] Nah, bunyi bungkusnya bikin ${c.reg.me} penasaran sama isinya`, demo: `[short pause] Buka ${c.produk}, baca label rasa, lalu periksa ${c.proof} ya`, cta: `Kalau penasaran, cek detailnya ada di keranjang kuning ya` }),
  (c) => ({ hook: `[whispers] Eh, dengarkan bunyinya pelan dulu sebelum menebak rasa`, demo: `[medium pause] Bunyi ${c.produk} memancing rasa penasaran; rasanya menunggu dicicipi sih`, cta: `Cari pilihan rasanya ada di keranjang kuning deh` }),
  (c) => ({ hook: `[whispers] Nah, ${c.reg.sapaan}, teksturnya sudah kedengaran dari bunyinya sih`, demo: `[long pause] Cek nama rasa, komposisi ${c.produk}, dan detail ${c.proof} loh`, cta: `Kalau ingin mencoba, buka rinciannya ada di keranjang kuning ya` }),
  (c) => ({ hook: `[whispers] Sumpah, [giggles] suara renyah ini bikin pengin buka bungkusnya`, demo: `[short pause] Dengarkan teksturnya, baca label ${c.produk}, lalu pertimbangkan ${c.proof} deh`, cta: `Pilihan rasanya ada di keranjang kuning deh` }),
];
...
const T06: CopyFn[] = [
  ...
  (c) => ({ hook: `Sumpah, satu pilihan langsung bikin mata berhenti`, demo: `Tentukan varian dari informasi ${c.proof}, bukan dugaan pribadi ya`, cta: `Temukan variannya ada di keranjang kuning deh ya` }),
];
...
const T07: CopyFn[] = [
  ...
  (c) => ({ hook: `[fast] ${c.reg.sapaan}, centang hanya setelah aksinya benar-benar kelihatan`, demo: `[medium pause] Nah, poin pertama: ${c.produk} punya ${c.proof}? Cari jawabannya pada barangnya langsung sih`, cta: `Manfaatnya ada di keranjang kuning ya` }),
...
const T08: CopyFn[] = [
  ...
  (c) => ({ hook: `Eh, simpan kondisi awal sebelum lanjut ke hari berikutnya`, demo: `Ikuti petunjuk ${c.produk}, lalu jadwalkan pemeriksaan ${c.proof} sih`, cta: `Waktunya ada di keranjang kuning deh` }),
  ...
  (c) => ({ hook: `Sumpah, dua waktu ini perlu catatan yang jelas`, demo: `Rencana awal dan akhir cukup memeriksa ${c.proof} pada ${c.produk} ya`, cta: `Catatannya ada di keranjang kuning deh` }),
];
...
const T10: CopyFn[] = [
  ...
  (c) => ({ hook: `[fast] Eh, pakai cahaya dan posisi yang sama dari awal`, demo: `[medium pause] Nah, gunakan informasi resmi ${c.produk} untuk memahami ${c.proof} sih`, cta: `Detail resminya ada di keranjang kuning deh` }),
  ...
  (c) => ({ hook: `[fast] Sumpah, [giggles] kedua kondisinya harus benar-benar setara`, demo: `[short pause] Oleskan ${c.produk} sesuai petunjuk pada area kecil, samakan cahayanya, lalu lihat ${c.proof} deh`, cta: `Panduannya ada di keranjang kuning deh` }),
];
...
const ADS_MEJA_KOSONG: CopyFn[] = [
  ...
  (c) => ({ hook: `[slow] Sumpah, [giggles] ruang kosong di meja ini harus punya alasan`, demo: `[short pause] Hitung [giggles] langkah kerja; verifikasi ${c.produk} melalui keterangan ${c.proof} deh`, cta: `Fiturnya ada di keranjang kuning deh` }),
];
...
const ADS_ATAP: CopyFn[] = [
  ...
  (c) => ({ hook: `[fast] Sumpah, [laughs] jalur masuknya kacau dan semua orang langsung menoleh`, demo: `[short pause] Sebelum heboh, cek kategori ${c.produk} serta rincian ${c.proof} deh`, cta: `Pilihan barangnya ada di keranjang kuning deh` }),
];
...
const PROMO_TERBATAS: CopyFn[] = [
  ...
  (c) => ({ hook: `[fast] Sumpah, [giggles] harga jujur nggak perlu menakut-nakuti`, demo: `[short pause] Cek ${c.proof} pada ${c.produk}; harga tercantumnya ${c.harga}, lalu bandingkan sendiri deh`, cta: `Nah, periksa harga produknya ada di keranjang kuning deh` }),
];
