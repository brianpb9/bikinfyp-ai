...
test("3 varian beda keluarga hook dan semua lolos validator strict", async () => {
  const variants = await generateScripts({ tanpaLlm: true, product, register: "bestie" });
...
test("100 generate: 100% lolos validator, 0% kata terlarang", async () => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const variants = await generateScripts({ tanpaLlm: true, product, register });
    for (const v of variants) {
...
test("register bunda tidak pernah mengandung gue/lo; genz tidak aku/kamu", async () => {
  for (const v of await generateScripts({ tanpaLlm: true, product, register: "bunda" })) {
    const full = v.segments.map((s) => s.text).join(" ").toLowerCase();
    assert.ok(!/\b(gue|gua|gw|lo|lu|elu)\b/.test(full), `bunda bocor gue/lo: ${full}`);
  }
  for (const v of await generateScripts({ tanpaLlm: true, product, register: "genz" })) {
    const full = v.segments.map((s) => s.text).join(" ").toLowerCase();
...
test("struktur segmen 0-3/3-10/10-15 dan caption+hashtag sesuai kontrak", async () => {
  const [v] = await generateScripts({ tanpaLlm: true, product, register: "netral" });
  assert.deepEqual(
...
test("CTA 'keranjang kuning' cuma untuk link TikTok; Shopee/tanpa sumber pakai 'keranjang' polos", async () => {
  ...
  const [tiktokVariant] = await generateScripts({ tanpaLlm: true, product: tiktokProduct, register: "netral" });
  ...
  const [shopeeVariant] = await generateScripts({ tanpaLlm: true, product: shopeeProduct, register: "netral" });
...
test("harga muncul eksplisit di hook atau demo untuk berbagai nominal", async () => {
  for (const price of [5000, 25000, 85000, 250000, 1500000]) {
    ...
    for (const v of await generateScripts({ tanpaLlm: true, product: p, register: "bestie" })) {
...
test("durasi 30 dtk: timing segmen skala 2x, demo diperpanjang, lolos validator (v1, 2026-08-03)", async () => {
  for (const qualityTier of ["silent_caption", "high_quality"] as const) {
    const variants = await generateScripts({ tanpaLlm: true, product, register: "bestie", qualityTier, durationSec: 30 });
    assert.equal(variants.length, 3);
    for (const v of variants) {
      assert.deepEqual(
        v.segments.map((s) => [s.role, s.start, s.end]),
    ...
    const [v30] = variants;
    const [v15] = await generateScripts({ tanpaLlm: true, product, register: "bestie", qualityTier });
...
test("durasi 45 dtk: timing segmen skala 3x, demo diperpanjang, lolos validator (v1.3, 2026-08-04)", async () => {
  for (const qualityTier of ["silent_caption", "high_quality"] as const) {
    const variants = await generateScripts({ tanpaLlm: true, product, register: "bestie", qualityTier, durationSec: 45 });
    assert.equal(variants.length, 3);
    for (const v of variants) {
      assert.deepEqual(
        v.segments.map((s) => [s.role, s.start, s.end]),
    ...
    const [v45] = variants;
    const [v30] = await generateScripts({ tanpaLlm: true, product, register: "bestie", qualityTier, durationSec: 30 });
