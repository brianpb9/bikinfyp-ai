...
test("C2 concurrency guards preserve E3/E7 provenance and validate locked admission rows", () => {
  ...
  assert.match(evidenceLease, /SELECT images,raw_meta,category,product_type_token,product_type_confirmed_token,product_type_confirmed_by,[\s\S]+product_type_state[\s\S]+FROM products/,
    "A2/A3 evidence lease does not reload rights binding plus complete C2 state");
  for (const route of [
