...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (postgresRuntimeEnabled()) await pgUpdateProduct(user.id, id, { name, priceIdr, category, productVisualDesc: visualDesc });
    else {
      db!.prepare("UPDATE products SET name = ?, price_idr = ?, category = ?, product_visual_desc = ? WHERE id = ?").run(name, priceIdr, category, visualDesc, id);
      audit(user.id, "product.updated", "products", id, { name, price_idr: priceIdr });
    }
