    ...
    try {
      const plans = (await pool.query<PlanRow>(
        `SELECT pp.id, pp.job_id, pp.channel, pp.scheduled_at, pp.caption, pp.status, pp.posted_at,
                p.name AS product_name,j.job_product_snapshot,p.category AS product_category,p.category_review_state,
      ...
      const ready = (await pool.query<{
        ...
        format: string; duration_s: number; caption: string | null; job_product_snapshot:string|null;
        product_category:string;category_review_state:string;category_review_reason:string|null;category_review_version:number;
      }>(
        `SELECT j.id AS job_id,j.job_product_snapshot,p.name AS product_name,p.category AS product_category,
                p.category_review_state,p.category_review_reason,p.category_review_version,o.video_url,j.created_at,
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const initial = await pool.query<{product_id:string}>(
        "SELECT product_id FROM jobs WHERE id=$1 AND org_id=$2 AND state='READY'",
      ...
      return await withProductEvidenceMutationLock(initial.rows[0].product_id,async()=>{
        const owned=await pool.query(`SELECT j.job_product_snapshot,p.category AS product_category,
          ...
          WHERE j.id=$1 AND j.org_id=$2 AND j.state='READY'`,[jobId,membership.org_id]);
        if (!owned.rows[0]) throw ERR.NOT_FOUND("Videonya");
        assertCurrentC5JobGeneration(owned.rows[0]);
